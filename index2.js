  ...
  try {
    ...
    if (retry && retryable) {
      ...
      try {
        ...
        logger.warn(`🔁 Retrying navigation at ${new Date().toISOString()}`);
        await delay(5000);
        try {
          await page.goto(url, { waitUntil: 'networkidle2', timeout });
          logger.log(`✅ Retry successful for: ${url}`);
        } catch (e2) {
          logger.error(`❌ Retry failed after ${Date.now() - startRetry} ms: ${e2.message}`);
        }
        await delay(10000);
        return true;
      } catch (e2) {
  ...
  while (true) {
    try {
      ...
      if (/ERR_TUNNEL_CONNECTION_FAILED|ECONNREFUSED|network|timeout|ERR_SOCKS_CONNECTION_FAILED|ERR_CONNECT_TIMEOUT/i.test(err && err.message ? err.message : '')) {
        ...
        if (localBrowser) await safeCloseBrowser(localBrowser, logger);
        localBrowser = null;
        localPage = null;
        await delay(5000);
...
(async () => {
  const numInstances = 7; // max parallel browsers desired
